...
function renderTaskList() {
  ...
  container.innerHTML = scheduledTasks.map(task => {
    ...
    if (task.status === 'success') {
      statusText = `下单成功 订单号:${task.orderNumber}`;
    ...
    return `
      <div class="task-item ${statusClass}">
        ...
        <div class="task-info">
          ...
          <div>商品ID: ${task.goodsId} | 区服: ${task.server} | 价格: ${task.price}元</div>
  ...
  for (const task of scheduledTasks) {
    ...
    if (timeUntil <= 5000 && timeUntil > -60000) { // 提前5秒，但不超过1分钟过期
      ...
      try {
        ...
        if (orderRes.success) {
          task.status = 'success';
          task.orderNumber = orderRes.result.orderNumber;
          task.statusMessage = '下单成功';
          log(`✔ 定时任务下单成功 订单号=${orderRes.result.orderNumber}`);
          
          await pushBark("定时下单成功", `${task.goodsName} | 订单号:${orderRes.result.orderNumber}`, `http://dms.52108.com/#/pages/shop/detail?id=${task.goodsId}`);
        } else {
          task.status = 'error';
...
async function manualCreateOrder() {
  ...
  try {
    ...
    if (res.success) {
      log(`✔ 手动下单成功 订单号=${res.result.orderNumber}`);
      showStatus(`手动下单成功 订单号=${res.result.orderNumber}`, 'success');
      ...
      for (const r of results) {
        ...
        if (autoOrderEl.checked) {
          try {
            ...
            if (orderRes.success) {
              log(`✔ 已自动下单 订单号=${orderRes.result.orderNumber}`);
